...
public:
    ...
    virtual void writeForest(string fileName) {
        ...
        //OctTreeNode* octreeRoot = binaryTreeToOctree(binaryRoot);
        fclose(fp);
    ...
    OctTreeNode* binaryTreeToOctree(BinaryTreeNode* binaryRoot) {
      if (binaryRoot == nullptr) {
          return nullptr;
        }

      OctTreeNode* octRoot = new OctTreeNode(binaryRoot->val);
      std::queue<BinaryTreeNode*> binaryQueue;
      std::queue<OctTreeNode*> octQueue;

      binaryQueue.push(binaryRoot);
      octQueue.push(octRoot);

      while (!binaryQueue.empty()) {
          BinaryTreeNode* binaryNode = binaryQueue.front();
          OctTreeNode* octNode = octQueue.front();

          binaryQueue.pop();
          octQueue.pop();

          if (binaryNode->left) {
              BinaryTreeNode* leftChild = binaryNode->left;
              OctTreeNode* octChild = new OctTreeNode(leftChild->val);
              octNode->children.push_back(octChild);
              binaryQueue.push(leftChild);
              octQueue.push(octChild);
          }

          if (binaryNode->right) {
              BinaryTreeNode* rightChild = binaryNode->right;
              OctTreeNode* octChild = new OctTreeNode(rightChild->val);
              octNode->children.push_back(octChild);
              binaryQueue.push(rightChild);
              octQueue.push(octChild);
          }
      }
